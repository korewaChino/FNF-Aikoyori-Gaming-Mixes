...
class PlayState extends MusicBeatState
{
	...
	public static var isAikoMode:Bool = false;
	...
	override public function create()
	{
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - AIKOMOD " + MainMenuState.aikoModVer : ""), 16);
		...
		else
		{
			...
			else if (isAikoMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					// if ()
					StoryMenuState.weekUnlocked[Std.int(Math.min(storyWeek + 1, AikoModeMenuState.weekUnlocked.length - 1))] = true;
					...
					FlxG.save.data.weekUnlocked = AikoModeMenuState.weekUnlocked;
					FlxG.save.flush();
				}
				else
				{
